...
function goTimer(time) {
    const timer = setInterval(() => {
        if (time >= 1) {
            ...
            stopProgress.style.animation = "animateProgress 5s linear infinite";
...
function goTimerError(time) {
    const timer = setInterval(() => {
        if (time >= 1) {
            ...
            errorProgress.style.animation = "animateErrorProgress 3s linear infinite";
...
getSignal.onclick = function () {
    let receivingSignal = getRandomFloat(1, 2.79, 2);
